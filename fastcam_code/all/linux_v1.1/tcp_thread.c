    ...
    while(1)
    {
        ...
        else
        {
            ...
            while(1)
            {
                ...
                int length_serv_frame_int = LengthAssembel(rv_buffer[9], rv_buffer[10]);
                for(int count = 0; count < (12 + length_serv_frame_int); ++count)
                {
                    printf("Message from client[%d]:%02hX\n", count, rv_buffer[count]);
                }

                ServerParamSolve(connect_fd, rv_buffer, (12 + length_serv_frame_int));
                memset(rv_buffer, 0x00, sizeof(rv_buffer));
